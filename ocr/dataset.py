...
class SequentialSampler(Sampler):
    ...
    def __iter__(self):
        dataset_indexes = np.random.choice(
            a=self.dataset_len,
            size=min(self.epoch_size, self.dataset_len),
...
def read_and_concat_datasets(csv_paths):
    ...
    for csv_path in csv_paths:
        ...
        csv_data['filename'] = csv_data['filename'].apply(get_full_img_path, csv_path=csv_path)
...
def get_data_loader(
    ...
    batch_size, drop_last, data_size=0
):
