...
def generate_random_date():
    ...
    month_scripted = random.choice(months)
    format = random.choice([1, 1, 1, 2, 3, 4, 5, 5, 6])
    if format == 1:
        ...
        date = f'{day:0>2} {month_scripted} {year}'
    elif format == 6:
        date = f'{day:0>2} {month_scripted} {year} г.'
    return date


def generate_dates(num: int):
    result = []
    for i in range(num):
        result.append(generate_random_date())
    return result


def generate_random_number():
    length = random.randint(1, 9)
    chars = [i for i in range(10)] * 5
    chars.extend(['/', '.', '-'])
    result = []
    for i in range(length):
        result.append(str(random.choice(chars)))
    return ''.join(result)
...
def generate_numbers(num):
    result = []
    for i in range(num):
        result.append(generate_random_number())
    return result


def generate_random_word(lang_dict):
    length = random.randint(1, 2)
    result = []
    for i in range(length):
        word = random.choice(lang_dict)
        cap = random.choice([1, 1, 1, 2, 2, 3])
        if cap == 1:
            ...
        elif cap == 2:
...
def generate_words(num, lang_dict):
    result = []
    for i in range(num):
        result.append(generate_random_word(lang_dict))
    return result


def generate_strings(num_strings: int, lang_dict: list):
    s = []
    s.extend(generate_dates(num_strings // 3))
    s.extend(generate_numbers(num_strings // 3))
    s.extend(generate_words(num_strings // 3, lang_dict))
    return s
...
def main():
    ...
    strings = generate_strings(args.count, lang_dict)
